...
#' @param neighborMatrix matrix; (optional) procomputed indices of nearest neigbors
...
return(list(neighborMatrix=neighborMatrix,community=community))
}
...
#' @importFrom RANN nn2
#' @export
find_neighbors <- function(data, k, eps = 0) {
  nearest <- nn2(data, data, k, searchtype = "standard", eps = eps)
  return(nearest[[1]])
}